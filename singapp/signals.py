...
def increase_comment_count(sender, instance, created, **kwargs):
    if created:
        # Comment가 Sing 모델에 연결되어 있는지 확인
        content_type = ContentType.objects.get_for_model(Sing)
        if instance.content_type == content_type:
            # ContentType이 Sing, 댓글 수를 증가시킴
            Sing.objects.filter(pk=instance.object_id).update(comment_count=F('comment_count') + 1)
...
def decrease_comment_count(sender, instance, **kwargs):
    ...
    content_type = ContentType.objects.get_for_model(Sing)
    if instance.content_type == content_type:
        # ContentType이 Sing, 댓글 수를 감소시킴
        Sing.objects.filter(pk=instance.object_id).update(comment_count=F('comment_count') - 1)

        
# 여기에 신호 리시버 함수를 작성합니다.
@receiver(post_save, sender=Sing)
...
@receiver(post_delete, sender=Sing)
