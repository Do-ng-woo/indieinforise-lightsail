...
def increase_comment_count(sender, instance, created, **kwargs):
    if created:
        # Comment가 Album 모델에 연결되어 있는지 확인
        content_type = ContentType.objects.get_for_model(Album)
        if instance.content_type == content_type:
            # ContentType이 Album, 댓글 수를 증가시킴
            Album.objects.filter(pk=instance.object_id).update(comment_count=F('comment_count') + 1)
