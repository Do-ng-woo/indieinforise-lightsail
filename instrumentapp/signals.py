...
def increase_comment_count(sender, instance, created, **kwargs):
    if created:
        # Comment가 Instrument 모델에 연결되어 있는지 확인
        content_type = ContentType.objects.get_for_model(Instrument)
        if instance.content_type == content_type:
            # ContentType이 Instrument, 댓글 수를 증가시킴
            Instrument.objects.filter(pk=instance.object_id).update(comment_count=F('comment_count') + 1)
...
def decrease_comment_count(sender, instance, **kwargs):
    ...
    content_type = ContentType.objects.get_for_model(Instrument)
    if instance.content_type == content_type:
        # ContentType이 Instrument, 댓글 수를 감소시킴
        Instrument.objects.filter(pk=instance.object_id).update(comment_count=F('comment_count') - 1)
